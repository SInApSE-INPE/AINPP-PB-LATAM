...
def main():
    ...
    for m in models:
        for lt in lead_times:
            ...
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "RMSE", "mean": np.random.uniform(5, 10)})
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "MAE", "mean": np.random.uniform(2, 5)})
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "SSIM", "mean": np.random.uniform(0.6, 0.9)})
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "STD_obs", "mean": 10.0})
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "STD_pred", "mean": np.random.uniform(7, 12)})
            records.append({"model": m, "lead_time": lt, "threshold": np.nan, "metric_name": "Correlation", "mean": np.random.uniform(0.5, 0.95)})
            
            # Categorical/Threshold dependent
            for t in thresholds:
                records.append({"model": m, "lead_time": lt, "threshold": t, "metric_name": "CSI", "mean": np.random.uniform(0.1, 0.5)})
                records.append({"model": m, "lead_time": lt, "threshold": t, "metric_name": "POD", "mean": np.random.uniform(0.3, 0.8)})
                records.append({"model": m, "lead_time": lt, "threshold": t, "metric_name": "FAR", "mean": np.random.uniform(0.1, 0.6)})
                records.append({"model": m, "lead_time": lt, "threshold": t, "metric_name": "FSS", "mean": np.random.uniform(0.3, 0.8)})
    ...
    B, H, W = 1, 256, 256
    target_map = np.random.lognormal(mean=0, sigma=1, size=(H, W)) # precipitation like distribution
    pred_map = target_map * 0.8 + np.random.normal(0, 0.5, (H, W))
    pred_map = np.clip(pred_map, 0, None)
    
    # Map Comparison (Sharpness / Error map)
    plot_comparison(target_map, pred_map, Path(out_str) / "sharpness_comparison.png", title="Spatial Error & Blur check")
