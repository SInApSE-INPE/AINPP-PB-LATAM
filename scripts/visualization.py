...
def test_runner_flow():
    # 1. Create Mock Data
    # Metrics
    summary = {"categorical": {"Thresh_0.5_POD": 0.8, "Thresh_0.5_SR": 0.7, "Thresh_0.5_FAR": 0.3}}
    ...
    np.savez_compressed(
        os.path.join(input_dir, "sample_0.npz"), target=target, prediction=prediction
    )

    # 2. Run Runner
    config = {"style": {"context": "paper"}, "maps": {"cmap": "viridis"}, "animation": {"fps": 2}}
