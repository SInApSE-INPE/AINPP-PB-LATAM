...
def generate_benchmark_figures(df: pd.DataFrame, output_dir: str):
    ...
    if not data_pod.empty and not data_far.empty and "threshold" in data_pod.columns:
        ...
        merged_cat = pd.merge(
            pod_agg, far_agg, on=["model", "threshold"], suffixes=("_POD", "_FAR")
        )
        ...
        for _, row in merged_cat.iterrows():
            plt.scatter(
                row["SR"], row[f"{val_col}_POD"], color=model_colors[row["model"]], s=50, zorder=5
            )
            plt.annotate(
                f"{row['threshold']}mm",
                (row["SR"], row[f"{val_col}_POD"]),
                xytext=(5, 5),
    ...
    if not data_csi.empty and "threshold" in data_csi.columns:
        for model in data_csi["model"].unique():
            ...
            if "lt_num" in df_model.columns:
                lt_order = (
                    df_model[["lead_time", "lt_num"]]
                    .drop_duplicates()
                    .sort_values("lt_num")["lead_time"]
                )
