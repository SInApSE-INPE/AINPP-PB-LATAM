...
class Inferencer:
    ...
    @torch.no_grad()
    def infer_single(
        self, input_tensor: torch.Tensor, base_timestamp: str, coords: Optional[dict] = None
    ) -> Path:
        """
        ...
        )  # Removes batch (Tout, C, H, W) or (Tout, H, W)
    ...
    def infer_historical(
        ...
        logger.info(
            f"Historical inference complete! [{current_idx} samples saved in {output_zarr}]"
        )
        return output_zarr
