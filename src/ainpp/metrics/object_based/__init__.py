...
class ObjectBasedMetrics:
    ...
    def compute(pred: np.ndarray, target: np.ndarray, threshold: float) -> dict:
        ...
        metrics = {"Object_POD": np.nan, "Object_FAR": np.nan, "Object_CSI": np.nan, "Centroid_Distance": np.nan, "Count_Bias": np.nan}
    ...
    def _compute_single_frame(pred_bin2d, target_bin2d):
        ...
        count_bias = num_pred / (num_target + EPS) if num_target > 0 else (np.nan if num_pred == 0 else np.inf)
        ...
        csi = hits / (hits + misses + false_alarms + EPS) if (hits + misses + false_alarms) > 0 else np.nan
